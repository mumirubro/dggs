...
async def process_single_card(card_info: str, max_retries: int = 3) -> Dict:
    ...
    while retry_count < max_retries:
        ...
        try:
            ...
            is_blocked = False
            for blocked in GLOBAL_STATE.blocked_responses:
                if blocked in result_msg:
                    is_blocked = True
                    break
            
            if is_blocked:
                GLOBAL_STATE.ban_site_permanently(site, f"Blocked response: {result_msg[:50]}...")
                retry_count += 1
                GLOBAL_STATE.update_stats('retry')
                continue # Retry with next site

            # Capture Order ID and Token if available
            order_id = ""
            order_token = ""
            if isinstance(result_msg, str):
                if "Order ID:" in result_msg:
                    try: order_id = result_msg.split("Order ID:")[1].split("\n")[0].strip()
                    except: pass
                if "Token:" in result_msg:
                    try: order_token = result_msg.split("Token:")[1].split("\n")[0].strip()
                    except: pass
