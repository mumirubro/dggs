...
async def msx_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def sem_process(card):
        async with semaphore:
            return await sx.process_single_card(card)

    results_data = []
    tasks = [sem_process(card) for card in cards]
    
    # Process all cards and gather results
    raw_results = await asyncio.gather(*tasks)
    
    for i, (card, result) in enumerate(zip(cards, raw_results), 1):
        if result['status'] == 'success':
            result_msg = result['message']
            # ... rest of your logic for notifications ...
...
async def bl_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if response not in sx.GLOBAL_STATE.blocked_responses:
        sx.GLOBAL_STATE.blocked_responses.append(response)
        sx.GLOBAL_STATE.save_config()
...
async def sbl_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sx.GLOBAL_STATE.blocked_responses:
        await update.message.reply_text("📂 Block list is empty")
        return
    resp = "📂 Blocked Responses:\n" + "\n".join([f"- {r}" for r in sx.GLOBAL_STATE.blocked_responses])
...
async def ubl_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        return
    response = " ".join(context.args)
    if response in sx.GLOBAL_STATE.blocked_responses:
        sx.GLOBAL_STATE.blocked_responses.remove(response)
        sx.GLOBAL_STATE.save_config()
...
"""
    if result.get('order_id'):
        response += f"𝐎𝐫𝐝𝐞𝐫 𝐈𝐃 ➜ <code>{result['order_id']}</code>\n"
    if result.get('order_token'):
        response += f"𝐓𝐨𝐤𝐞𝐧 ➜ <code>{result['order_token']}</code>\n"

    response += f"""𝑽𝑩𝑽 ➜ {html.escape(vbv_info)}
𝐫𝐞𝐚𝐬𝐨𝐧/𝐭𝐲𝐩𝐞 ➜ N/A
━━━━━━━━━
"""
